/*
  # Schema inicial do Inscribo CRM - Versão Segura
  
  Esta versão verifica se os objetos já existem antes de criá-los
  para evitar conflitos com execuções anteriores.
*/
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_institution_id') THEN
        CREATE INDEX idx_profiles_institution_id ON profiles(institution_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_leads_institution_id') THEN
        CREATE INDEX idx_leads_institution_id ON leads(institution_id);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_leads_current_stage') THEN
        CREATE INDEX idx_leads_current_stage ON leads(current_stage);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_visits_lead_id') THEN
        CREATE INDEX idx_visits_lead_id ON visits(lead_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_visits_scheduled_date') THEN
        CREATE INDEX idx_visits_scheduled_date ON visits(scheduled_date);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_interactions_lead_id') THEN
        CREATE INDEX idx_interactions_lead_id ON interactions(lead_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_stage_changes_lead_id') THEN
        CREATE INDEX idx_stage_changes_lead_id ON stage_changes(lead_id);
    END IF;
END $$;

-- Triggers para updated_at (só cria se não existir)
DO $$
BEGIN
...
-- Políticas RLS (só cria se não existir)
DO $$
BEGIN
    -- Políticas para institutions
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated users can read institutions') THEN
        CREATE POLICY "Authenticated users can read institutions" ON institutions FOR SELECT TO authenticated USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admins can manage institutions') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read lead stages') THEN
        CREATE POLICY "Users can read lead stages" ON lead_stages FOR SELECT TO authenticated USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admins can manage lead stages') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read leads from their institution') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update leads from their institution') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read visits from their institution') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage visits') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read interactions from their institution') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create interactions') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read stage changes from their institution') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create stage changes') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admins can manage webhooks') THEN
...
-- Trigger para criar perfil automaticamente (só cria se não existir)
DO $$
BEGIN
...
-- Inserir dados iniciais (só se não existir)
...
-- Inserir etapas padrão do funil (só se não existir)
